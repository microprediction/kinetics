...
When an alternative disappears or changes, where does its demand go? The
models used to answer this, essentially universally, are the logit family:
multinomial logit, nested logit, and above all random-coefficients logit in
the BLP tradition, the workhorse of demand estimation in industrial
organization, antitrust, and pricing \cite{berry1995,train2009,conlon2020}.
They are universal for one reason. They can be computed. Choice modeling did
not start there: it started with Thurstone's 1927 model \cite{thurstone1927}
of jointly Gaussian utilities with unrestricted correlation, known in
econometrics as multinomial probit (MNP).
...
restricted, and the share-inversion apparatus that demand estimation is
built on exists on the logit side only. In the substitution study of
Section~\ref{sec:boundary}, plain logit misallocates about 23\% of the
demand released by a deleted alternative. The correctly structured probit
model misallocates 5\%.
