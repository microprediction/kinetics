...
shares match observed shares, has been out of reach at scale. We give an
...
there, reaching ten thousand alternatives in eleven minutes, where the same
calibration through our implementation of the standard simulator
extrapolates to weeks at matched accuracy. Conditionally on the factors the
...
lattice computes all shares in 21 seconds; our GHK baseline at matched
accuracy extrapolates, doubly (wall time in $N$, draws by $R^{-1/2}$), to
roughly fifty hours.
...
share map is strictly monotone on the quotient, and shares determine
utilities uniquely. And symmetry plus positive definiteness make every
...
\;\to\; \infty
\]
as $\|\mu\| \to \infty$ on $\mathbf{1}^\perp$. Since $D_i > 0$ the
...
as expected for analytic, rapidly decaying integrands under a uniform rule,
and the $L$-error is subdominant. Holding $L$
...
\approx p_{\max}/\tau^2$ draws per evaluation. At $N = 5000$ and the
measured direct-simulation rate, matching this solver's model-side
consistency ($\tau \sim 10^{-6}$) prices one forward evaluation in weeks
and the calibration in months to a year; even $\tau = 10^{-5}$ prices the
