...
alternatives in eight seconds on a laptop, and five thousand in under a
minute. Its cost grows approximately linearly, reaching ten thousand
alternatives in minutes, where extrapolation from our plain per-alternative
simulator implementation puts matched-accuracy simulation-based calibration
on a weeks scale. Conditionally on the factors the
...
negligible at the production resolutions tested. One further distinction, raised by a careful
...
differences. So the Laplacian form is an extremely accurate symmetric surrogate at the
...
$10^{-8}$ at production resolution. Finally, the implemented inversion rebuilds
...
is $O(QN(k+L))$, reducing to $O(QNL)$ for $k \ll L$. The slopes in
...
the reference's own maximum-coordinate noise is of comparable order, so
these entries resolve the lattice error only to that order). A replication study at
...
$(6.8\times10^{-3}/5\times10^{-4})^2 \approx 185$), against 21 seconds
--- an order-of-magnitude illustration, not a forecast: it assumes the
$N=200$ error constant, and GHK's $R=1000$ error measured directly at
$N=1000$ is larger ($9.2\times10^{-3}$, 59 s), so the assumption favors
the baseline; the seed-to-seed spread (Figure~\ref{fig:frontier}) adds a
further factor-few uncertainty. The baseline is a straightforward
...
$0.004$--$0.024$). Self-convergence checks at large $N$ show stability below what the
...
error bound, since both resolutions could share bias). Two genuine bounds
...
$N=1000$ in 8 seconds and $N=5000$ in 45 seconds, and its
Chebyshev-separated forward variant computes all 5000 shares in 83
milliseconds at $6\times10^{-5}$ accuracy, a measured $255\times$ over
the reference pass; the single-threaded NumPy timings above remain the
reported baseline.

Scaling is approximately linear in $N$ over the tested range (experiment
19; at fixed spatial resolution the lattice would grow with the utility
range, $O(\sqrt{\log N})$ under Gaussian utilities): calibration takes 48, 120, 330,
...
0.22]$, positive definite as proved. The gap to simulation here is structural, and simple arithmetic shows why
(arithmetic, not a lower bound). With common random numbers the simulated
share map is a fixed deterministic function that can be inverted to any
solver tolerance; the limitation is that it is the wrong map. Its
coordinatewise integration error is of order $\sqrt{p_{\max}/R}$, so
making the simulated map itself $\tau$-accurate costs $R \approx
p_{\max}/\tau^2$ draws per evaluation, and inverting it exactly still
leaves utilities whose true shares miss the target at scale $\tau$,
amplified through $J^{-1}$. Under independent direct simulation at the
measured $N = 5000$ throughput, this variance calculation prices a
$\tau \sim 10^{-6}$ forward map in weeks per evaluation and the
calibration in months to a year; variance reduction and common random
numbers improve the constants but not the $\tau^{-2}$ scaling, and a
maximum over $N$ coordinates adds a further $\log N$ factor in
high-probability statements. At the crude $\tau = 10^{-4}$ where
...
set. The quadrature route's accuracy against the exact map is set by
...
signs); it is not certified to minimize a projected norm ($P\,\mathrm{diag}(D)\,P$
is not diagonal), so the repository also implements the certified
alternating fit on the reduced quotient (top-$k$ PSD step for the loadings,
nonnegative least squares for $D$): on the boundary matrices it changes the
quotient residual by under 1\%, so the heuristic is not the binding
constraint. The covariance-quotient residual $\|P(\Sigma -
...
$6\times10^{-5}$ at $45\times$ speed for $N=5000$ in NumPy, and the
compiled version of the same construction measures $255\times$ (83 ms for
