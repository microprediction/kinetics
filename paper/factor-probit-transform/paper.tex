...
lattice computes all shares in 7 seconds; the extrapolated matched-accuracy
...
resolve to genuine small positives) while a simulation reference can
measure relative error only for shares above roughly $50/R$; committed
records therefore carry both metrics. A maximum over $N$ coordinates tends to grow with $N$ at fixed
...
$(6.8\times10^{-3}/5\times10^{-4})^2 \approx 185$), against 7 seconds.
...
calibrates $N=1000$ in 3.1 seconds and $N=5000$ in 17.5 seconds.
