...
twenty. Cost grows roughly linearly: ten thousand alternatives take forty
seconds. The computed shares carry log-odds
errors of a few parts in a hundred thousand, hundred-to-one tails
included; matching that on a single hundred-to-one share would take
direct utility simulation about forty billion draws, or weeks, with the
cost growing in inverse proportion to the share toward the tail.
\end{abstract}
...
(GHK) simulator \cite{geweke1994,hajivassiliou1998,train2009}, the
econometric branch of the Genz separation-of-variables family
\cite{genz1992} whose RQMC form (Genz--Bretz) is the default in
...
points. All 5000 shares of an $N = 5000$ problem take 7 seconds, where our
...
factor quadrature \cite{butler1982,elrod1995} & $N$ separate $(k{+}1)$-dim integrals & no \\
...
lattice transform near $N \approx 200$; at equal running time its error is
about 23 times larger (Table~\ref{tab:frontier}), so at equal error it is
roughly 500 times slower. By $N = 5000$ the lattice computes all shares in
7 seconds and the simulator needs about fifty hours at matched accuracy:
the lattice is roughly twenty-five thousand times faster
...
\cite{hotz1993}, surjectivity is Norets--Takahashi \cite{norets2013}, and
Berry, Gandhi and Haile \cite{bgh2013} derive demand invertibility from
...
\paragraph{Numerical method.} The benchmarks use $L = 501$ lattice
...
evidence about the hard regime).
...
\midrule
5 & 7 ms, err $5.7\times10^{-4}$ & err $1.8\times10^{-3}$ & 1 ms, err $3.1\times10^{-3}$ \\
20 & 29 ms, err $4.4\times10^{-4}$ & err $6.5\times10^{-4}$ & 12 ms, err $4.4\times10^{-3}$ \\
50 & 71 ms, err $2.7\times10^{-4}$ & err $1.0\times10^{-3}$ & 74 ms, err $7.9\times10^{-3}$ \\
200 & 290 ms, err $3.0\times10^{-4}$ & err $1.1\times10^{-3}$ & 1300 ms, err $6.8\times10^{-3}$ \\
1000 & 1.4 s, err $6.8\times10^{-4}$\,$\dagger$ & err $1.1\times10^{-3}$ & --- \\
5000 & 7.0 s, err $5.0\times10^{-4}$\,$\dagger$ & --- & --- \\
\bottomrule
\end{tabular}
...
lattice $3.9\times10^{-4}$ versus direct MC $1.1\times10^{-3}$), not the
...
$(6.8\times10^{-3}/5\times10^{-4})^2 \approx 185$), against 7 seconds: at
equivalent accuracy the lattice is roughly twenty-five thousand times
faster.

This is an order-of-magnitude illustration, not a forecast. It assumes the
...
In log-odds the comparison must name its target, and the two natural
targets give opposite verdicts. Matched to the coarsest level our own
simulation reference can certify ($3\times10^{-2}$, its noise floor, on
shares above $10^{-3}$), direct utility simulation gets there in minutes:
at coarse accuracy on head shares, simulation is competitive and the
lattice's factor-thirty advantage there is not the story. The story is
the lattice's actual accuracy. Against a super-converged reference
(Gauss--Hermite $60^2$, $L=5001$; experiment 25) the production lattice's
log-odds error is at most $5\times10^{-5}$ on every share from $10^{-9}$
to one. Simulation needs $1/(p\epsilon^2)$ draws to match relative error
$\epsilon$ on a share $p$: matching $5\times10^{-5}$ on a single
hundred-to-one share costs $4\times10^{10}$ draws, about three weeks at
the measured $N=5000$ draw rate, and every decade further into the tail
multiplies that by ten.

\paragraph{The baseline.} A plain single-threaded Python GHK:
per-alternative Cholesky, plain pseudorandom uniforms, one realization per
problem, shares normalized by their sum. It establishes the scaling of the
per-alternative simulation approach, not a field-wide impossibility.

A compiled (Rust) implementation of the same baseline, in the repository,
runs within a factor of two of the NumPy version at these sizes: the
baseline's cost is intrinsic to the per-alternative sequential
construction, not a Python artifact.
...
We ran it at wall time matched to the lattice call (experiment
16, both methods scored against the same fresh $2\times10^6$-draw
reference): its maximum-coordinate error was $1.8$--$5.6\times$ the
lattice's at every tested $N$ ($1.1\times10^{-3}$ versus
$3.9\times10^{-4}$ at $N=1000$, $0.2$M draws in the matched 1.4
seconds). At the $10^{-3}$ accuracy level,
then, raw forward speed does not by itself separate the methods.

\paragraph{The fair factor-structured baseline.} Conditioning on the
factors and the alternative's own shock makes each probability a
$(k{+}1)$-dimensional integral,
\[
p_i = \E_{f,z} \prod_{j \ne i}
\Phi\!\left(\frac{\mu_i - \mu_j + (v_i - v_j)^\top f +
\sqrt{D_i}\,z}{\sqrt{D_j}}\right),
\]
evaluable by scrambled-Sobol RQMC per alternative: the obvious modern
competitor that uses exactly the structure this paper assumes. It still
evaluates $N$ separate integrals, $O(RN^2)$ in total, where the shared
field couples them into one $O(QN(k+L))$ pass.

Measured (experiment 24; identity verified against the lattice to
$1.7\times10^{-5}$ before comparison): at $N=50$ it matches the lattice's
accuracy in comparable time, but at $N=1000$ it needs 215 seconds against
the lattice's 1.5 to reach the same $4\times10^{-4}$, a $142\times$ gap
that widens quadratically. That coupling, not any weakness of GHK, is the
computational content of the paper.
...
simulation, 2.6 seconds to $2.0\times10^{-4}$ ($2^{20}$ points).
...
seconds: both land in the same accuracy class, well off the lattice.
...
completes in 19 seconds. The iteration solves its own equations to
...
independent $5\times10^6$-draw targets: 18 seconds each, 161--162
...
Its Chebyshev-separated forward variant computes all 5000 shares in 77
milliseconds at $6\times10^{-5}$ accuracy, a measured $93\times$ over the
reference pass. The single-threaded NumPy timings above remain the
reported baseline.
...
range, $O(\sqrt{\log N})$ under Gaussian utilities): calibration takes
16, 41, 110, and 201 seconds at $N = 1000$, $2000$, $5000$, $10000$, a
flat 11--15
...
further $\log N$ factor in high-probability statements.
...
\cite{geweke1994,hajivassiliou1998,train2009}; Genz \cite{genz1992}
developed the same separation-of-variables construction in numerical
statistics, where its RQMC form is the default MVN evaluator. Ridgway
\cite{ridgway2016} reads GHK as sequential importance sampling, proves its
variance can deteriorate exponentially, and builds SMC alternatives; Botev
\cite{botev2017} sharpened the family with minimax tilting; Cao, Genton,
Keyes and Turkiyyah \cite{cao2021} push MVN probabilities to thousands of
...
Deterministic approximations run from Mendell--Elston \cite{mendell1974}
(surprisingly competitive when optimally ordered \cite{connors2014})
through Bhat's MACML \cite{bhat2011} to expectation propagation:
Cunningham, Hennig and Lacoste-Julien \cite{cunningham2011} found EP
highly accurate precisely for rectangular regions, and Fasano and Denti
\cite{fasano2025} evaluate generic Gaussian CDFs by EP on a dual probit
...
Inversion theory is likewise established. Hotz and Miller \cite{hotz1993}
inverted choice probabilities to utility differences; Norets and Takahashi
\cite{norets2013} prove surjectivity of the utility-to-probability map;
Berry, Gandhi and Haile \cite{bgh2013} obtain demand invertibility from
...
before any comparison (repository experiments 13--25; a single manifest,
...
12 (2021) 295--317.
\bibitem{hotz1993} V.~J. Hotz, R.~A. Miller. Conditional choice
probabilities and the estimation of dynamic models. \emph{Review of
Economic Studies} 60(3):497--529, 1993.

\bibitem{bgh2013} S.~Berry, A.~Gandhi, P.~Haile. Connected substitutes
...
\bibitem{genz1992} A.~Genz. Numerical computation of multivariate normal
probabilities. \emph{J.\ Computational and Graphical Statistics} 1 (1992)
141--149.
\bibitem{ridgway2016} J.~Ridgway. Computation of Gaussian orthant
probabilities in high dimension. \emph{Statistics and Computing} 26 (2016)
899--916.
\bibitem{cao2021} J.~Cao, M.~G. Genton, D.~E. Keyes, G.~M. Turkiyyah.
...
(2021) 2.
\bibitem{mendell1974} N.~R. Mendell, R.~C. Elston. Multifactorial
qualitative traits: genetic analysis and prediction of recurrence risks.
\emph{Biometrics} 30 (1974) 41--57.
\bibitem{cunningham2011} J.~P. Cunningham, P.~Hennig, S.~Lacoste-Julien.
Gaussian probabilities and expectation propagation. arXiv:1111.6832 (2011).
\bibitem{fasano2025} A.~Fasano, F.~Denti. Expectation propagation for
