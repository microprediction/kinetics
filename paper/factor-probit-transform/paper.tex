...
further factor-few uncertainty.

\paragraph{The baseline.} A plain single-threaded Python GHK: per-alternative Cholesky, plain pseudorandom uniforms, one realization
per problem, shares normalized by their sum. It establishes the scaling of
the per-alternative simulation approach, not a field-wide impossibility.
...
\paragraph{Where the methods separate: error decay.} Experiment 17 sweeps
...
\paragraph{Every method on one problem.} Figure~\ref{fig:frontier}
(left) shows each method at its most accurate tested setting, all timed on
the same problem against one common reference. The lattice reaches the
reference's noise floor ($4.1\times10^{-5}$) in 0.03 seconds. Direct MC
needs 15.7 seconds to reach $1.2\times10^{-4}$ ($R = 10^7$); Sobol direct
simulation, 2.6 seconds to $2.0\times10^{-4}$ ($2^{20}$ points). GHK
reaches $2.0\times10^{-3}$ in 10.7 seconds ($R = 10^4$), with seed-to-seed
spread $[3.9\times10^{-3}, 1.3\times10^{-2}]$ at $R = 10^3$. QMC-GHK
improves plain GHK five-to-sevenfold at equal $R$ ($4.2\times10^{-4}$ in
7.0 seconds), and minimax tilting \cite{botev2017}, implemented and
anchor-validated (experiment 22), reaches $5.0\times10^{-4}$ in 38
seconds: both land in the same accuracy class, well off the lattice. Below
$10^{-3}$ the deterministic map separates decisively, consistent with
$R^{-1/2}$ against a rapidly converging quadrature.
