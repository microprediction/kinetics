...
the number of alternatives: roughly a million times faster than plain
simulation-in-the-loop calibration at matched accuracy. The computed shares carry log-odds
...
largest-deletion stratum of one test design, plain logit misallocates
about 23\% of the demand
...
\midrule
GHK simulation \citep{geweke1994,hajivassiliou1998} & $N$ separate runs, $R^{-1/2}$ error & no reported large-$N$ share inversion known to us \\
...
(we report $\max_i |\log \hat p_i - \log p_i^{\mathrm{ref}}|$ over
resolvable shares, which agrees with log-odds error to $O(p)$ on small
shares), and there the log-domain lattice is at its strongest (deep-tail shares
...
\paragraph{Log-odds assessment.} Against super-converged references
...
An equal-marginal-variance replication (normalized loadings
$\|v_i\|^2 = \rho$, idiosyncratic variance $1 - \rho$, multiple
seeds) is experiment 29.
...
figure; this draft is built from commit \texttt{9efd63d} (an
