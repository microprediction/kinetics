...
probit model by \citet{hausman1978} and known since as MNP.
...
integrals where the shared field gives all $N$ in one pass. In
...
covariances but simulated probabilities; \citet{elrod1995}
...
per class. The racing literature posed both
...
generalized-entropy duality, without an algorithm; \citet{berry1995} built demand
...
compiled version of the same construction measures $93\times$, with
exponential convergence in $r$. Development to full
...
the border at intermediate spectral decay. We leave to future work
...
econometrics \citep{hausman1978}, that constraint is gone: all shares, their derivatives, and
the inverse map cost seconds at ten thousand alternatives. Estimating the
factors themselves remains open, and with $(V, D)$ in hand the choice
...
sample sizes. \emph{Statistics and Computing} (2026).
