...
\textbf{this paper} & one $O(QNL)$ pass & Thm.~\ref{thm:inv}; 8 s at $N{=}1000$ \\
\bottomrule
\end{tabular}
\end{center}
...
projecting: shares sum to one identically, so $\mathbf{1}^\top
(\partial p / \partial \theta) = 0$, and $\mathbf{1}^\top J = 0$ by
symmetry of $J$ (Theorem~\ref{thm:inv}) with $J\mathbf{1} = 0$; both terms
