...
alternatives in three seconds on a laptop, and five thousand in under
twenty. Cost grows roughly linearly: ten thousand alternatives take minutes.
...
\textbf{this paper} & one $O(QNL)$ pass & Thm.~\ref{thm:inv}; 3 s at $N{=}1000$ \\
\bottomrule
\end{tabular}
\end{center}
...
($k=2$ factors). The upper GHK line rescales $R$ by the squared error ratio ($R^{-1/2}$
scaling) so both methods deliver the same accuracy; the matched-accuracy
extrapolation is developed in Section~\ref{sec:benchmark}. Dashed segments are power-law
...
nonsingular (established for this model in Theorem~\ref{thm:inv}) it
...
$10^{-8}$ at production resolution. The implemented inversion rebuilds the lattice envelope from the current
...
at equal $R$ and still well off the lattice frontier. Minimax tilting
\cite{botev2017}, implemented and anchor-validated in the repository
(experiment 22), reaches $5.0\times10^{-4}$ at $R = 10^4$ in 38 seconds:
the accuracy class of QMC-GHK, at higher cost in this per-alternative
implementation. Below $10^{-3}$ the deterministic map separates decisively,
consistent with $R^{-1/2}$ against a rapidly converging quadrature.
...
$0.004$--$0.024$). Numerical well-posedness matches
Theorem~\ref{thm:inv}: the reduced Jacobian assembled from $N$ JVP calls
at $N=50$ is symmetric to $1.7\times10^{-18}$, has row-sum defect
$6.2\times10^{-17}$, and reduced eigenvalues in $[2.5\times10^{-5},
0.22]$, positive definite as proved. Self-convergence checks at large $N$ show stability below what the
...
$N=1000$ in 8 seconds and $N=5000$ in 45 seconds at the conservative $L =
1501$. The lattice is overkill there: $L = 501$ is machine-precision
identical ($3\times10^{-17}$, consistent with the $L$-sweep of experiment
17) and brings calibration to 3.1 seconds at $N=1000$ and 17.5 seconds at
$N=5000$, with identical iteration counts, residuals, and recovery. The
...
The gap to simulation here is structural, and simple arithmetic shows why
...
before any comparison (repository experiments 13--23; a single manifest,
