...
twenty. Cost grows roughly linearly: ten thousand alternatives take minutes. For
scale, one matched-accuracy evaluation of the five-thousand-share vector
alone, extrapolated from our like-for-like implementation of the standard
simulator, is a fifty-hour computation. Conditionally on the factors the
...
draw-argmax simulator at wall time matched to the lattice call and is scored
against experiment 16's fresh $2\times10^6$-draw references, under which
the lattice's own errors are $3.0$--$3.9\times10^{-4}$ at every size;
cross-column comparisons should use that common reference (at $N=1000$:
lattice $3.9\times10^{-4}$ versus direct MC $6.2\times10^{-4}$), not the
noisier dagger entries. Reference: $2\times10^6$ MC draws
...
16, both methods scored against the same fresh $2\times10^6$-draw
reference): its maximum-coordinate error was $1.5$--$4\times$ the
lattice's at every tested $N$ ($6.2\times10^{-4}$ versus
$3.9\times10^{-4}$ at $N=1000$, $0.5$M draws in the matched 4.1
seconds). At the $10^{-3}$ accuracy level,
...
\caption{Left: each method at its most accurate tested setting, timed on
the same $N=200$, $k=2$ problem against one common reference (twin
$5\times10^7$-draw simulations; experiment 17). The shaded band is the
reference's own noise: the lattice point sits at that floor, and its true
error is $\sim10^{-8}$ by self-convergence. Right: factor-quadrature refinement at fixed $L$: Gauss--Hermite
...
wrong map. Its coordinatewise error against the exact map is of order
...
target at scale $\tau$, amplified through $J^{-1}$. Under plain
...
weeks per evaluation and the calibration in months to a year. Common
random numbers do not change the plain-MC rate; QMC and specialized
Gaussian integration can (both are benchmarked in
Section~\ref{sec:benchmark}), and a maximum over $N$ coordinates adds a
further $\log N$ factor in high-probability statements. At the crude $\tau = 10^{-4}$ where
...
factor-to-idiosyncratic variance ratio; because the implementation is
min-wins, the base is the \emph{reflected} (minimum) Gumbel corresponding
to a standard maximum-Gumbel in utility coordinates, and the committed
anchor verifies that its zero-loading case equals softmax at $N=12$ to
$2.8\times10^{-17}$; the candidate factor law is Gaussian in all cases (the truth's $t(5)$ factor law is misspecified for
...
bars). The score per deletion block $B$ is the
