...
seconds. At equivalent log-odds accuracy on
those five thousand shares, our transform is thirty times faster than
direct utility simulation and twenty-five thousand times faster than the
standard per-alternative simulator.
\end{abstract}
...
The same exercise in log-odds, matched to the lattice's measured
$3\times10^{-2}$ at $N=1000$ (itself reference-noise limited), spreads the
alternatives out. At $N=5000$ direct utility simulation gets there in
about four minutes (41\,s at $N=1000$ from the measured $7.1\times10^{-2}$
at $10^6$ draws, times five), QMC-GHK in a few hours, and plain GHK in
about fifty hours (its measured $R=1000$ log-odds error at $N=200$ is
$4.6\times10^{-1}$, so $R^{-1/2}$ scaling costs a factor 212 before the
$(5000/200)^2$ size scaling). Against the lattice's 7-second pass that
makes the lattice thirty, eighteen hundred, and twenty-five thousand
times faster respectively. Matching its certified tail behavior rather
than its measured error would multiply every simulation entry by the
$1/(p\epsilon^2)$ factor of the smallest share of interest.
...
before any comparison (repository experiments 13--24; a single manifest,
