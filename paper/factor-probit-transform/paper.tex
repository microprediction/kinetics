...
object has re-emerged in machine learning: heteroscedastic image
classifiers place exactly the factor-Gaussian utility of this paper on
their final layer and take the argmax, approximating the winner
probabilities by Monte Carlo over a softmax relaxation
\citep{collier2021}.
...
product applies the Hessian $\nabla^2 G$, all in $O(QN(k+L))$. The bijection itself is essentially known:
...
\sqrt{\mathrm{KL}\big(\mathcal{N}(B^\top\mu, B^\top\Sigma B)\,\|\,
\mathcal{N}(B^\top\mu, B^\top\hat\Sigma B)\big)/2}$, with the Gaussian
KL in closed form. Measured (experiment 26), the bound is roughly two
...
$8.4\times10^{-3}$ (actual $9.7\times10^{-5}$). It converts a
covariance-fit residual into a hard share-error guarantee before any
transform is run.
...
experiments 13--26). A single manifest,
...
\emph{Journal of the American Statistical Association} (2025).
